...
os.environ["WANDB_PROJECT"] = "multiple-ner-style-transfer"
...
@hydra.main(version_base="1.3", config_path="../configs", config_name="multiple_ner_train.yaml")
def main(cfg: DictConfig):
    ...
    # merge all train_ds parquet hf datasets
    train_ds: datasets.Dataset = concatenate_datasets(
        [
            load_dataset(
                "parquet",
                data_files=f"hf_datasets/mimic_iii_ner/data/ner-{dataset_name}-train.parquet",
            )["train"]
            for dataset_name in ["0.06-0", "0.06-1-ofzh3aqu", "0.06-2-ofzh3aqu"]
        ]
    )
    test_ds: datasets.Dataset = load_dataset(
        "parquet",
    ...
    def add_ner_tags(example):
        tokens_size_cumulated[0] += len(example["labels"])
