#!/usr/bin/env python3
"""
Data Loader for Clinical Documents Pipeline
"""
...
class ClinicalDataLoader:
    ...
    def _load_data(self):
        ...
        try:
            ...
            if initial_count != final_count:
                self.logger.info(f"Filtered out {initial_count - final_count} documents shorter than 100 characters")
    ...
    def get_random_document_pairs(self, 
                                  count: int = BATCH_SIZE,
                                  same_subject: bool = False,
                                  category_filter: Optional[List[str]] = None,
                                  min_text_length: int = 200) -> List[DocumentPair]:
        """
        ...
        if category_filter:
            ...
            self.logger.debug(f"Filtered to {len(filtered_df)} documents matching categories: {category_filter}")
        
        if min_text_length > 0:
            filtered_df = filtered_df[filtered_df['text'].str.len() >= min_text_length]
            self.logger.debug(f"Filtered to {len(filtered_df)} documents with min length {min_text_length}")
        ...
        for i in range(count):
            if same_subject:
                ...
                if not subjects_with_multiple:
                    self.logger.warning("No subjects with multiple documents, falling back to random pairing")
    ...
    def create_balanced_pairs(self, 
                              count: int = BATCH_SIZE,
                              balance_categories: bool = True) -> List[DocumentPair]:
        """
