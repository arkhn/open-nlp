#!/usr/bin/env python3
"""
Clinical Document Conflict Pipeline - Main Execution Script
"""
...
def main():
    ...
    batch_parser.add_argument('--same-subject', action='store_true', 
                             help='Try to pair documents from the same subject')
    batch_parser.add_argument('--categories', nargs='+', 
                             help='Filter by document categories (e.g. "Discharge summary" "Progress note")')
    batch_parser.add_argument('--max-retries', type=int, default=3,
                             help='Maximum retry attempts for validation failures (default: 3)')
    batch_parser.add_argument('--min-score', type=int, default=70,
                             help='Minimum validation score required (default: 70)')
    
    # Test conflict type command
    test_parser = subparsers.add_parser('test-conflict', help='Test a specific conflict type')
    test_parser.add_argument('conflict_type', help='Conflict type to test (e.g. opposition, anatomical, value)')
    test_parser.add_argument('--count', type=int, default=3, help='Number of document pairs to test (default: 3)')
    ...
    try:
        ...
        if args.command == 'batch':
            ...
            print(f"Moderator Agent: {stats['agents']['moderator']['name']} (min score: {stats['agents']['moderator']['min_validation_score']})")
