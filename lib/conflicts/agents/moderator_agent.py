#!/usr/bin/env python3
"""
Moderator Agent - Validates modifications made to clinical documents
"""
...
from config import MODERATOR_AGENT_PROMPT, CONFLICT_TYPES
...
class ModeratorAgent(BaseAgent):
    ...
    def process(self, 
                ...
                conflict_type: str) -> ValidationResult:
        """
        ...
        try:
            ...
            prompt = MODERATOR_AGENT_PROMPT.format(
            ...
            for field in required_fields:
                if field not in parsed_response:
                    self.logger.warning(f"Missing field '{field}' in Moderator response, using default")
            ...
            try:
                ...
                self.logger.warning(f"Invalid validation score '{validation_score}', defaulting to 0")
                validation_score = 0
            ...
            if validation_score < self.min_validation_score:
                is_valid = False
                self.logger.info(f"Validation score {validation_score} below threshold {self.min_validation_score}, marking as invalid")
            ...
            result = ValidationResult(
                ...
                issues_found=issues_found,
                approval_reasoning=parsed_response.get('approval_reasoning', 'No reasoning provided')
            )
            
            processing_time = time.time() - start_time
            
            self.logger.info(f"Moderator Agent completed validation in {processing_time:.2f}s")
            self.logger.info(f"Validation result: {'VALID' if result.is_valid else 'INVALID'} (Score: {result.validation_score}/100)")
    ...
    def detailed_validation_check(self, 
                                  original_pair: DocumentPair, 
                                  modified_docs: EditorResult) -> Dict[str, Any]:
        """
        ...
        checks = {
            ...
            'length_analysis': {
                ...
                'doc1_length_change': len(modified_docs.modified_document1) - len(original_pair.doc1_text),
                'doc2_original_length': len(original_pair.doc2_text),
                'doc2_modified_length': len(modified_docs.modified_document2),
                'doc2_length_change': len(modified_docs.modified_document2) - len(original_pair.doc2_text),
            },
            'content_analysis': {
        ...
        checks['documents_modified']['any_changed'] = (
            checks['documents_modified']['doc1_changed'] or 
            checks['documents_modified']['doc2_changed']
        )
        ...
        if abs(checks['length_analysis']['doc1_length_change']) > len(original_pair.doc1_text) * 0.5:
            checks['potential_issues'].append("Document 1 length changed by more than 50%")
            
        if abs(checks['length_analysis']['doc2_length_change']) > len(original_pair.doc2_text) * 0.5:
