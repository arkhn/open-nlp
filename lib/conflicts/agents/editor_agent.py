#!/usr/bin/env python3
"""
Editor Agent - Modifies clinical documents to introduce specified conflicts
"""
...
from config import EDITOR_AGENT_PROMPT, CONFLICT_TYPES
...
class EditorAgent(BaseAgent):
    ...
    def process(self, document_pair: DocumentPair, conflict_instructions: ConflictResult) -> EditorResult:
        """
        ...
        self.logger.info(f"Editor Agent modifying documents to create '{conflict_instructions.conflict_type}' conflict")
        ...
        try:
            ...
            # Prepare prompt with documents and modification instructions
            ...
            if (parsed_response['modified_document1'].strip() == document_pair.doc1_text.strip() and 
                parsed_response['modified_document2'].strip() == document_pair.doc2_text.strip()):
                
                self.logger.warning("Editor Agent returned documents without modifications, retrying...")
    ...
    def _retry_with_emphasis(self, document_pair: DocumentPair, conflict_instructions: ConflictResult) -> EditorResult:
        """
        ...
        # Add emphasis to the prompt
        emphasized_prompt = EDITOR_AGENT_PROMPT.format(
            conflict_type=conflict_type_info.name,
            conflict_description=conflict_type_info.description,
        ...
        return EditorResult(
            ...
            changes_made=parsed_response.get('changes_made', "Retry attempt - modifications emphasized")
        )
    ...
    def validate_modifications(self, original_pair: DocumentPair, modified_result: EditorResult) -> Dict[str, Any]:
        """
        ...
        validation = {
            ...
            'length_change_doc1': len(modified_result.modified_document1) - len(original_pair.doc1_text),
            'length_change_doc2': len(modified_result.modified_document2) - len(original_pair.doc2_text),
            'has_any_changes': False,
            'changes_summary': modified_result.changes_made
        }
        
        validation['has_any_changes'] = validation['has_changes_doc1'] or validation['has_changes_doc2']
