#!/usr/bin/env python3
"""
Doctor Agent - Analyzes clinical documents and determines conflict types to introduce
"""
...
from config import DOCTOR_AGENT_PROMPT, CONFLICT_TYPES
...
class DoctorAgent(BaseAgent):
    ...
    def process(self, document_pair: DocumentPair) -> ConflictResult:
        ...
        self.logger.info(f"Doctor Agent analyzing document pair: {document_pair.doc1_id} & {document_pair.doc2_id}")
        
        try:
            # Prepare prompt with conflict types and documents
            conflict_types_formatted = format_conflict_types_for_prompt(CONFLICT_TYPES)
            
            prompt = DOCTOR_AGENT_PROMPT.format(
            ...
            if parsed_response['conflict_type'] not in CONFLICT_TYPES:
                self.logger.warning(f"Unknown conflict type '{parsed_response['conflict_type']}', defaulting to 'opposition'")
